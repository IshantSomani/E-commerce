                ...
                {reversedCartItems.length === 0 ? (
                    ...
                    <div className="space-y-6 select-none">
                        {reversedCartItems.map((item) => (
                            <div key={item._id} className="flex flex-col md:flex-row items-center bg-white h-[67vh] shadow-lg rounded-lg overflow-hidden">
                                <img src={`${import.meta.env.VITE_API_URI}/${item.productImage}`} alt={item.productName} className="w-full md:w-1/4 object-cover select-none" />
